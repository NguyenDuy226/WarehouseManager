import { HttpClient, HttpParams } from '@angular/common/http';
import { inject, Injectable } from '@angular/core';
import { Observable } from 'rxjs';
import { EntityStatus } from './material-service';
import { environment } from '../../../environments/environment';
import { PagedResult, PagingRequest } from '../Auth Service/user-servive';

export interface UnitOfMeasureDto {
  id: string;
  code: string;
  name: string;
  status: EntityStatus;
}

export interface CreateUnitOfMeasureDto {
  name: string;
}

export interface UpdateUnitOfMeasureDto {
  name: string;
  status: EntityStatus;
}

@Injectable({
  providedIn: 'root',
})
export class UnitOfMeasureService {
  private readonly apiUrl = `${environment.apiUrl}/UnitOfMeasures`;
  private readonly http = inject(HttpClient);

  getAll(request: PagingRequest): Observable<PagedResult<UnitOfMeasureDto>> {
    let params = new HttpParams()
      .set('pageNumber', request.pageNumber.toString())
      .set('pageSize', request.pageSize.toString());

    if (request.keyword) params = params.set('keyword', request.keyword.trim());
    if (request.sortBy) params = params.set('sortBy', request.sortBy);
    if (request.sortDirection) params = params.set('sortDirection', request.sortDirection);

    return this.http.get<PagedResult<UnitOfMeasureDto>>(this.apiUrl, { params });
  }

  getById(id: string): Observable<UnitOfMeasureDto> {
    return this.http.get<UnitOfMeasureDto>(`${this.apiUrl}/${id}`);
  }

  create(dto: CreateUnitOfMeasureDto): Observable<UnitOfMeasureDto> {
    return this.http.post<UnitOfMeasureDto>(this.apiUrl, dto);
  }

  update(id: string, dto: UpdateUnitOfMeasureDto): Observable<boolean> {
    return this.http.put<boolean>(`${this.apiUrl}/${id}`, dto);
  }

  delete(id: string): Observable<boolean> {
    return this.http.delete<boolean>(`${this.apiUrl}/${id}`);
  }
}